...
- `/api/v1/transcription` – upload recorded audio and proxy Gladia's Speech-to-Text API
...
> Speech-to-text now flows through Gladia. The frontend records microphone audio and posts it to `/api/v1/transcription`, so you must set `GLADIA_API_KEY` before running locally or inside Docker. HTTPS (or `localhost`) is still required for microphone access, and the backend continues to need `OPENAI_API_KEY` for TTS synthesis.
